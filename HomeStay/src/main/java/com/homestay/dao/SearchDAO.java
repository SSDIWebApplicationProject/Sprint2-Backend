...
public class SearchDAO implements ISearchDAO{
	...
	public ArrayList<Room>  getRoomDetails() {
	    ...
	    try{  
		 con= (Connection) Conn.getConnection();  
		 System.out.println(noOfGuestsUI);
		ps=con.prepareStatement("select * from room where maxPeople= "+getNoOfGuestsUI()+ " and cityId in (select cityId from City where CityName = '"+getCityName()+"')"); 
		System.out.println(ps);
		ResultSet rs=ps.executeQuery(); 
		while (rs.next()) {
			int id = rs.getInt("room_id");
